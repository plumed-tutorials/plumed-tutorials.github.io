...
In the conformational analysis, we will evaluate the convergence of chosen Collective Variables (CV's), characterize the CV space and estimate errors. In the protonation analysis, we will estimate average protonations of the titrable site at each pH value, estimate pKas using the Henderson-Hasselbalch equation and calculate errors, while using the metadynamics weights. Furthermore, we will compute the same properties using a [WHAM](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) procedure to reweight to obtain smoother and more accurate descriptions of several observables. 
...
[MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) STRUCTURE=data/${sys}.pdb 
# Define the atoms for the chi torsion angle CV 
chi: [TORSION](https://www.plumed.org/doc-master/user-doc/html/_t_o_r_s_i_o_n.html) ATOMS=8,9,19,28 
# Define the atoms for the sugar puckering CV 
puck: [PUCKERING](https://www.plumed.org/doc-master/user-doc/html/_p_u_c_k_e_r_i_n_g.html) ATOMS=6,8,9,13,11 
...
metad: [METAD](https://www.plumed.org/doc-master/user-doc/html/_m_e_t_a_d.html) ARG=chi,puck.Zx PACE=100000000 HEIGHT=0.0 BIASFACTOR=8 SIGMA=0.35,0.35 FILE=data/HILLS_pH${pH} GRID_MIN=-pi,-pi GRID_MAX=pi,pi RESTART=YES # <- this is the new stuff! 

# Using the REWEIGHT_BIAS, we provided the metad.bias as an argument to obtain the weights. 
as: [REWEIGHT_BIAS](https://www.plumed.org/doc-master/user-doc/html/_r_e_w_e_i_g_h_t__b_i_a_s.html) ARG=metad.bias 

# Unbiased histograms for the observables are obtained by providing the logweights previously defined. 
hhchi: [HISTOGRAM](https://www.plumed.org/doc-master/user-doc/html/_h_i_s_t_o_g_r_a_m.html) ARG=chi STRIDE=1 GRID_MIN=-pi GRID_MAX=pi GRID_BIN=100 BANDWIDTH=0.1 LOGWEIGHTS=as 
hhpuck: [HISTOGRAM](https://www.plumed.org/doc-master/user-doc/html/_h_i_s_t_o_g_r_a_m.html) ARG=puck.Zx STRIDE=1 GRID_MIN=-pi GRID_MAX=pi GRID_BIN=100 BANDWIDTH=0.1 LOGWEIGHTS=as 
# Then we can convert the histograms h(s) to free energies F(s) = -kBT * log(h(s)) 
# by using the [CONVERT_TO_FES](https://www.plumed.org/doc-master/user-doc/html/_c_o_n_v_e_r_t__t_o__f_e_s.html) module and then dump the free energies. 
ffchi: [CONVERT_TO_FES](https://www.plumed.org/doc-master/user-doc/html/_c_o_n_v_e_r_t__t_o__f_e_s.html) GRID=hhchi 
ffpuck: [CONVERT_TO_FES](https://www.plumed.org/doc-master/user-doc/html/_c_o_n_v_e_r_t__t_o__f_e_s.html) GRID=hhpuck 
# Print out the free energies F(s) 
[DUMPGRID](https://www.plumed.org/doc-master/user-doc/html/_d_u_m_p_g_r_i_d.html) GRID=ffchi FILE=ffchi_${pH}.dat 
[DUMPGRID](https://www.plumed.org/doc-master/user-doc/html/_d_u_m_p_g_r_i_d.html) GRID=ffpuck FILE=ffpuck_${pH}.dat 

# Print to a COLVAR file the chi angle, the Zx component of the sugar puckering and the metad.bias 
[PRINT](https://www.plumed.org/doc-master/user-doc/html/_p_r_i_n_t.html) ARG=chi,puck.Zx,metad.bias FILE=COLVAR_REWEIGHT_${pH} STRIDE=1 
EOF 
