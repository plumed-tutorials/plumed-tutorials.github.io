...
The hySAS ONEBEAD mapping (1 bead for 1 amino acid, 3 beads for 1 nucleodide) requires two PDBs, one for the [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and one for the [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) actions. This applies to both [SAXS](https://www.plumed.org/doc-master/user-doc/html/_s_a_x_s.html) and SAS modules. The [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) PDB may contain atoms, residues, chains, that are not included for the SAS signal calculation, while the [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) PDB must contain only the residues that are used for the ONEBEAD conversion. Some general rules: 

#### 1. Consistency between [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) PDBs 
The numbering of atoms and residues must be consistent between the [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) PDBs. For example, if [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) is provided with a PDB containing 100 residues, but the SAS calculation is performed on residues in the range 20-80, it is necessary to provide [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) with a PDB containing only these residues, while maintaining their numbering: if the 20th residue starts with atom number 319 and the 80th residue ends with atom number 1325, the [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) PDB must start with ATOM 319 and residue number 20, and end with ATOM 1325 and residue number 80. 
...
#### 2. Consistency between [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and the molfile 
...
#### 3. [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) PDB numbering 
The [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) PDB must start with ATOM number 1. The residue number can start with a positive number other than 1. 
#### 4. Providing the same file to [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) 
It is possible to provide [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) with the same PDB, taking into account all three points above. 
#### 5. PDB format 
...
``` 
Warning: The `-d` `--add-missing-atoms` flags combination adds the hydrogen atoms to the PDB even if they already exist with a different nomenclature. This means that this step could double the number of atoms in the PDB. A solution could be to remove the original hydrogens from the PDB allowing the script to add them again with the correct nomenclature. Note that the order of the atoms in the [MOLINFO](https://www.plumed.org/doc-master/user-doc/html/_m_o_l_i_n_f_o.html) and [TEMPLATE](https://www.plumed.org/doc-master/user-doc/html/_t_e_m_p_l_a_t_e.html) PDBs, as well as in a previously generated trajectory to be analysed with the driver, must be consistent. 
