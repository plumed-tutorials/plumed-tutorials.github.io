...
using the commands above the version of PLUMED that is found in the [PATH](https://www.plumed.org/doc-master/user-doc/html/_p_a_t_h.html) is used. If the version of PLUMED in your path is not the same 
...
<p> Lastly note that even if you do not want to use [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) it may still be useful to look at the modulefile as this file will tell 
...
This options is useful if you do not want to set up modules. As detailed below, however, we believe that using [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) is more flexible.</p> 
</div> 
...
(e.g. <code>+openblas</code>) and to enable all the optional [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) (<code>+allmodules</code>). 
...
<ul> 
<li> Follow the advice given below about using [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) and the module file that PLUMED provides to set up the environment. N.B you will need to edit this file to make it suitable for your environment.</li> 
<li> Compile with all the [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) enabled so that users can use the full range of features. In addition, ensure that features that are provided by specific libraries are enabled. For example, users employing gromacs will likely want to use libxdrfile to write trr/xtc files. If gromacs is installed before PLUMED, and if the location of libxdrfile is provided to PLUMED during the configure stage then this feature will be available.</li> 
...
<li> Keep track of the version of PLUMED you used to patch each of the MD codes. You might do this by giving the MD code [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) names such as <code>gromacs/4.6.7p2.2.0</code> to indicate that this is gromacs patched with PLUMED 2.2.0. By keeping track of this information you make it more straightforward for users to report errors on our forum.</li> 
...
contains the path to all the [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) loaded at compilation time. When PLUMED is compiled using the <a onclick='openModal("enablerpath")'>--enable-rpath</a> 
...
<p>If, however, you are compiling the code for yourself only then it may be better to only compile those [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) you specifically need. 
If you would like to compile with the core [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) and the dimred and ves [modules](https://www.plumed.org/doc-master/user-doc/html/_w_h_a_m.html) you could use the command <code>--enable-modules=dimred:ves</code>.</p> 
</div> 
</div></div> 
